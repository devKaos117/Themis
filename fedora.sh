...
main() {
	...
	if [[ ! -f "/etc/yum.repos.d/brave-browser.repo" ]]; then dnf config-manager addrepo --from-repofile="https://brave-browser-rpm-release.s3.brave.com/brave-browser.repo"; fi
	# ====== LibreWolf
	# https://librewolf.net/
	if [[ ! -f "/etc/yum.repos.d/librewolf.repo" ]]; then dnf config-manager addrepo --from-repofile="https://repo.librewolf.net/librewolf.repo"; fi
	# ====== Microsoft
	# https://learn.microsoft.com/pt-br/powershell/scripting/install/install-rhel?view=powershell-7.5
	# https://code.visualstudio.com/docs/setup/linux
	rpm --import "https://packages.microsoft.com/keys/microsoft.asc"
	if [[ ! -f "/etc/yum.repos.d/vscode.repo" ]]; then echo -e "[code]\nname=Visual Studio Code\nbaseurl=https://packages.microsoft.com/yumrepos/vscode\nenabled=1\nautorefresh=1\ntype=rpm-md\ngpgcheck=1\ngpgkey=https://packages.microsoft.com/keys/microsoft.asc" | tee /etc/yum.repos.d/vscode.repo > /dev/null ; fi
	if [[ ! -f "/etc/yum.repos.d/microsoft-prod.repo" ]]; then packaging::install "https://packages.microsoft.com/config/rhel/$(if [ $sysinfo::_OS_VERSION -lt 8 ]; then echo 7 ; elif [ $sysinfo::_OS_VERSION -lt 9 ]; then echo 8; else echo 9; fi )/packages-microsoft-prod.rpm" "rpm"; fi
	# ====== Docker
	if [[ ! -f "/etc/yum.repos.d/docker-ce.repo" ]]; then dnf config-manager addrepo --from-repofile "https://download.docker.com/linux/fedora/docker-ce.repo"; fi
	# ====== Update
	packaging::update
	...
	if packaging::install btop ; then
		...
		if curl -sSf "https://raw.githubusercontent.com/devKaos117/Themis/refs/heads/main/files/btop.conf" -o /tmp/btop.conf; then
			cp "/tmp/btop.conf" "$(getent passwd 1000 | cut -d : -f 6)/.config//btop/btop.conf" || logger::warning "Failed to set up user's btop.conf"
			cp "/tmp/btop.conf" "$(getent passwd 0 | cut -d : -f 6)/.config//btop/btop.conf" || logger::warning "Failed to set up root's btop.conf"
			rm /tmp/btop.conf
		else
			logger::warning "Failed to download btop.conf"
		fi
	fi
	# ====== Information
	...
	sh <(curl -fsSL "https://d2lang.com/install.sh") || logger::error "Failed to install D2"
	# ====== TexLive
